document.addEventListener('DOMContentLoaded', () => {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if (!response.ok) {
                    ...
                    console.error(`takeSnap attempt ${attempt} failed with status ${response.status}: ${errorData.error}`);
                    throw new Error(errorData.error || `HTTP ${response.status}`);
                }
                const data = await response.json();
                snapUrl = data.url;
                console.log(`Snap saved successfully: ${snapUrl}`);
                openEditor();
                return;
            } catch (error) {
    ...
    const recordVideo = async () => {
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                console.error(`recordVideo failed with status ${response.status}: ${errorData.error}`);
                throw new Error(errorData.error || 'Failed to record video');
            }
            const data = await response.json();
            console.log(`Video saved successfully: ${data.url}`);
    ...
    const recordBoomerang = async () => {
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                console.error(`recordBoomerang failed with status ${response.status}: ${errorData.error}`);
                throw new Error(errorData.error || 'Failed to record boomerang');
            }
            const data = await response.json();
            console.log(`Boomerang saved successfully: ${data.url}`);
            addSnap(data.url, true, data.type);
        } catch (error) {
            console.error('Error recording boomerang:', error);
            alert(`Failed to record boomerang: ${error.message}`);
        } finally {
    ...
    const loadSnaps = async () => {
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                console.error(`loadSnaps failed with status ${response.status}: ${errorData.error}`);
                throw new Error(errorData.error || 'Failed to load snaps');
            }
            const data = await response.json();
            snapsContainer.innerHTML = '';
            const snapsToShow = gallery.classList.contains('full-screen') ? data.snaps : data.snaps.slice(0, 3);
            ...
            alert(`Failed to load snaps: ${error.message}`);
        }
    };
    ...
    const saveSnap = async () => {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if (!response.ok) {
                    const errorData = await response.json();
                    console.error(`saveSnap attempt ${attempt} failed with status ${response.status}: ${errorData.error}`);
                    throw new Error(errorData.error || `HTTP ${response.status}`);
                }
                const data = await response.json();
                console.log(`Edited snap saved successfully: ${data.url}`);
                addSnap(data.url, true, data.type);
